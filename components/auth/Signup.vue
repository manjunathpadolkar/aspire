<template>
  <div>
    <div class="w-screen h-screen flex justify-center items-center bg-white">
      <form
        class="p-8 bg-white rounded-lg flex justify-center items-center flex-col shadow-lg border border-grey-100"
      >
        <div class="mb-8 mt-4">
          <svg
        ...
        <div class="w-full">
          <input
            ...
            type="email"
            name="email"
            class="mt-5 p-3 w-full focus:border-ap-color-#01D167 rounded border-2 outline-none text-ap-color-#0C365A"
            autocomplete="off"
            placeholder="Email"
            required
          />
        ...
        <div class="w-full">
          ...
          />
          <div 
            v-if="validation.hasError('confirm_password')" 
            class="block w-full text-left"
            :class="{ 'text-red-700': true }"
          >
        ...
        <button
          class="bg-ap-color-#01D167 text-white font-bold p-3 w-80 text-ap-font-size-19 mt-10 mb-4 text-center tracking-wide rounded-full"
          id="login"
          type="submit"
        ...
        <NuxtLink
          to="/login"
          class="text-ap-color-#01D167 text-sm mb-6"
          >Already registered? signin here.</NuxtLink
        >
      </form>
    </div>
  </div>
...
export default {
  ...
  validators: {
    ...
    'confirm_password, password': function (confirm_password, password) {
      let _this = this
      if (_this.registering || _this.validation.isTouched('confirm_password')) {
        return Validator.value(confirm_password).required().match(password);
      }
    }
  },
  methods: {
